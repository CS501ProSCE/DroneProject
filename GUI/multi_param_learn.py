...
def multi_param_learn(param_list,param_weights,datapath):
    ...
    labels = {2: 'Impact (Front Left)', 3: 'Impact (Front Right)', 4: 'Impact (Back Left)',
              5: 'Impact (Back Right)',
              6: 'Gust (from Left)', 7: 'Gust (from Right)', 8: 'Gust (from front)'}
    colors={2: "red", 3:"firebrick", 4:"orange", 5:"yellow", 6:"blue", 7:"navy", 8:"royalblue"}
    ...
    return "", x_train_final, y_train_final, labels,param_weights,para_weights,param_list,colors
...
def test_data_plot(param_list,x_test,colors,event_labels):
    ...
    for i, r in enumerate ([i for i in range(len(x_test))]):
        ...
        plt.plot(x_test[r], label=event_labels[i],color=colors[i],linewidth=2)
...
def training_data_plot(param_list,x_train,y_train,labels):
    ...
    colors = ["red", "firebrick","orange","yellow","blue","navy","royalblue",'#D62728', '#2C9F2C', '#FD7F23', '#1F77B4', '#9467BD',
    ...
    label_list=[]
    for key in labels:
        label_list.append(y_train.index(key))
    j=0

    for i, r in enumerate(label_list):
         a = math.ceil(len(label_list)/ 2)
         plt.subplot(a, 2, i + 1)
         plt.plot(x_train[r], label=labels[y_train[r]], color=colors[j], linewidth=2)
         j=j+1
         plt.xlabel('Samples @50Hz')
         plt.legend(loc='upper left')
         plt.tight_layout()
